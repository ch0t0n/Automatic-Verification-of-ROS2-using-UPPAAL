...
def Add_network_of_TA(root, ros_model):    
    ...
    for top in ros_model['Topic']:
        ...
        for pub in pubt:
            ...
            enqueue_text = ''
            for sub in subt:
                enqueue_text += 'enqueue(A'+sub+',n'+sub+',c'+sub+',o'+sub+','+x_val+'),'
            enqueue_text = enqueue_text[:-1]
            enqueue_p = ET.SubElement(trans_top, "label", kind="assignment")
            enqueue_p.text = enqueue_text
            x_range = ros_model['Trange'][top]
            select_p = ET.SubElement(trans_top, "label", kind="select")
            select_p.text = x_val+' : int['+x_range+']'


def main():
    # Reading the ros model from a text file
    f1, f2 = 'example_ros_model', 'kobuki_model'
    filename = f1
    ...
    tree.write(filename+'22.xml', encoding='utf-8', xml_declaration=True)
